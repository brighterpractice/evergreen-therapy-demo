(() => {
  ...
  try {
    ...
    /*
     * Evergreen retains its existing analytics event
     * vocabulary while sharing the v2 visitor/session
     * implementation used by the Brighter Sites fleet.
     */
    ...
    const eventForLink = (link) => {
      const explicit =
        link.getAttribute(
          'data-bs-analytics-event'
        );

      let eventName =
        allowedEvents.has(explicit)
          ? explicit
          : null;
