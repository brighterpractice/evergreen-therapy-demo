(() => {
  ...
  const newSession = () => {
    const timestamp = now();
    const session = { id: uuid(), createdAt: timestamp, lastActivityAt: timestamp, entrySent: false };
  ...
  if (!noindex) {
    ...
    if (session) {
      ...
      if (saveSession(session)) send({ event_name: 'page_view', is_entry: isEntry, referrer_host: getReferrerHost(), device_category: getDeviceCategory() });
    }
  }
  ...
  document.addEventListener('click', (event) => {
    ...
    if (eventName === 'appointment_click') {
      ...
      if (url.origin === window.location.origin && normalClick) {
        ...
        send({ event_name: eventName, ...extra }, true).catch(() => {}).finally(() => window.location.assign(url.href));
        return;
      }
    }
